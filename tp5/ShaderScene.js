...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/flat_half.vert", "shaders/flat_half.frag"),
		...
		this.testShaders[11].setUniformsValues({ normScale: 1, timeFactor: 0 });
	...
	update(t) {
		...
		if (this.selectedExampleShader == 6)
			...
			this.testShaders[11].setUniformsValues({ timeFactor: t / 70 % 100 });
		}
